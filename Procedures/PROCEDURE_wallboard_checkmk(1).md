...
### Étape 0 — Installation d'Ubuntu Server sur le PC (remplacement de Windows)

Si le PC fourni est encore sous Windows, on efface le disque pour installer Ubuntu Server à la place. Cette machine étant dédiée à l'affichage, aucune double amorce (dual boot) n'est nécessaire.

**a) Créer la clé USB bootable**

- Télécharger l'image ISO d'Ubuntu Server sur [ubuntu.com/download/server](https://ubuntu.com/download/server)
- Télécharger [Rufus](https://rufus.ie/) (sous Windows) ou [balenaEtcher](https://www.balena.io/etcher/) (multiplateforme)
- Brancher une clé USB (8 Go minimum, son contenu sera entièrement effacé)
- Dans Rufus/balenaEtcher : sélectionner l'ISO téléchargée, sélectionner la clé USB, lancer la création (schéma de partition **GPT** si le PC est en UEFI, ce qui est le cas de la plupart des PC récents)

> ⚠️ **Attention**
> Bien vérifier que c'est la bonne clé USB qui est sélectionnée avant de lancer l'écriture : l'opération efface tout son contenu sans possibilité de récupération.

**b) Démarrer sur la clé USB**

- Brancher la clé USB sur le PC, l'allumer
- Appuyer sur la touche du menu de démarrage (Boot Menu) au tout début du démarrage — généralement `F12`, `F11`, `Échap` ou `F2` selon le fabricant (Dell : F12, HP : F9, Lenovo : F12)
- Sélectionner la clé USB dans la liste des périphériques de démarrage

**c) Installer Ubuntu Server**

- Choisir la langue et la disposition du clavier
- Configurer le réseau : si un câble Ethernet est branché et qu'un serveur DHCP répond, l'IP sera attribuée automatiquement à ce stade (l'IP fixe sera configurée à l'étape suivante si besoin)
- À l'étape de partitionnement, choisir **"Use an entire disk"** (utiliser tout le disque) : cela efface Windows et toutes les données présentes
- Créer le compte administrateur initial (nom de machine, identifiant, mot de passe) — ce compte servira à l'administration, distinct du compte `kiosk` créé plus loin
- Cocher l'installation du serveur OpenSSH pendant l'installation, pour pouvoir s'y connecter à distance dès le premier démarrage
- Laisser l'installation se terminer, retirer la clé USB, redémarrer

**d) Vérifier l'accès après le premier démarrage**

Depuis un autre poste du réseau :

```bash
ssh <utilisateur>@<ip_du_pc>
```

Si la connexion fonctionne, la base Ubuntu Server est prête pour la suite de la procédure.
